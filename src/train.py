...
from monai.transforms import AddChannel, Compose, RandRotate90, Resize, ScaleIntensity, ToTensor,RandAdjustContrast
...
DEBUG = False
kernel_type = 'monai3d_resnet34_amp_160_3ch_1e-5_20ep_aug'
image_size = 160
use_amp = True
data_dir = '../input/train-jpegs'
num_workers = 4
init_lr = 1e-5
out_dim = 9
freeze_epo = 0
warmup_epo = 1
cosine_epo = 2 if DEBUG else 39
...
def run(fold):
    ...
    train_loader = torch.utils.data.DataLoader(dataset_train, batch_size=64, sampler=RandomSampler(dataset_train), num_workers=num_workers)
    valid_loader = torch.utils.data.DataLoader(dataset_valid, batch_size=64, num_workers=num_workers)
